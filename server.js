...
// Simple terrain height lookup for mobs
const SEA=90, H=180;
function mobGroundHeight(x){
  // Approximate surface height using same formula as client
  const seed=SEED;
  const s1=Math.sin(x*.05+seed)*Math.sin(x*.02+seed*1.7)*.5+.5;
  const s2=Math.sin(x*.12+seed)*.5+.5;
  let h=SEA-Math.floor(s1*16+s2*8+Math.sin(x*.008+seed)*10);
  return Math.max(15,Math.min(H-15,h));
}

const MOB_SPEEDS={zombie:.018,skeleton:.022,slime:.012,bat:.03,spider:.025,demon:.015,goblin:.028,vampire:.022};
...
setInterval(()=>{
  ...
  mobs.forEach(mob=>{
    ...
    const spd=MOB_SPEEDS[mob.type]||.018;
    if(nearest&&nd<30){
      const dx=nearest.x-mob.x;
      mob.vx+=(dx>0?1:-1)*spd;
    } else {
      if(Math.random()<.02)mob.vx=(Math.random()-.5)*spd*2;
    }
    // Cap speed
    mob.vx=Math.max(-spd*2,Math.min(spd*2,mob.vx));
    mob.vx*=0.82;
    mob.x+=mob.vx;
    ...
    if(!MOB_FLY.has(mob.type)){
      mob.vy=(mob.vy||0)+0.4;
      mob.y+=mob.vy/18;
      const groundY=mobGroundHeight(Math.floor(mob.x))-1;
      if(mob.y>=groundY){mob.y=groundY;mob.vy=0;}
      if(mob.y<0)mob.y=0;
    } else {
      // Flying mobs hover at player height
      if(nearest)mob.y+=(nearest.y-3-mob.y)*0.05;
    }
    mob.y=Math.max(0,Math.min(H-3,mob.y));
    // Barrier
    if(mob.x<1)mob.x=1;
  });
  mobs=mobs.filter(m=>m.hp>0);
  while(mobs.length<35)spawnMob();
  broadcast({type:'mob_update',mobs:mobs.map(m=>({id:m.id,type:m.type,x:Math.round(m.x*10)/10,y:Math.round(m.y*10)/10,hp:m.hp,maxHp:m.maxHp}))});
},250);
