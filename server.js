  ...
  ws.on('message',raw=>{
    ...
    switch(msg.type){
      ...
      case 'move':{
        ...
        const dx=Math.abs((msg.x||0)-p.x),dy=Math.abs((msg.y||0)-p.y);
        p.x=msg.x;p.y=msg.y;p.facing=msg.facing||1;p.anim=msg.anim||'idle';
        // Only broadcast if moved or changed animation
        if(dx>0.05||dy>0.05||msg.facing!==p.facing||msg.anim!==p.anim){
